...
int getValidSex(char* mensaje,char* mensajeError,char* output,int reintentos)
{
    int i = 0;
    int retorno = -1;
    if(mensaje != NULL && mensajeError != NULL && output != NULL && reintentos > 0)
    {
        retorno=0;
        do{
            *output = toupper(getChar(mensaje));
            i++;
        }while(*output != 'F' && *output != 'M' && i < reintentos );
    }
...
/**
 * \brief Verifica si el valor recibido contiene solo letras y que el primer char no sea un espacio
 * \param str Array con la cadena a ser analizada
 * \return 1 si contiene solo ' ' y letras y 0 si no lo es
 *
 */
...
/**
 * \brief Solicita un nombre al usuario y lo devuelve
 * \param mensaje Es el mensaje a ser mostrado
 * \param input Array donde se cargará el texto ingresado
 * \return 1 si el texto contiene solo letras
 */
...
int getValidNames(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getStringName(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
